function datesSet_add = Add_dates_pre(dt, n)
    ...
    for i = 0:n
        ...
        if weekday(datesSet_add(i+1)) == 7
            datesSet_add(i+1) = datesSet_add(i+1) - days(1);  % Sabato → Lunedì
        elseif weekday(datesSet_add(i+1)) == 1
            datesSet_add(i+1) = datesSet_add(i+1) - days(2);  % Domenica → Lunedì
        end
    end
end
